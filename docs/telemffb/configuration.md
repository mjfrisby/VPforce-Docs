...
- **Enable DirectInput Devices**
    ...
    - List generic DirectInput force-feedback devices (shown with a `[DI]` prefix) alongside VPforce hardware in the device selectors. Requires the separately distributed DInput bridge utility; a status line below the toggle reports the installed bridge version. Most VPforce users can leave this off; it exists to drive non-VPforce hardware.
